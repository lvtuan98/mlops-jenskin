...
class DataIngestion:
    ...
    def run(self):
        ...
        try:
            ...
            # Check if the csv file is already present in the local directory
            if os.path.exists(LOCAL_RAW_FILE_PATH):
                logger.info(f"CSV file is already present in {LOCAL_RAW_FILE_PATH}")
                shutil.copy(LOCAL_RAW_FILE_PATH, RAW_FILE_PATH)
            else:
